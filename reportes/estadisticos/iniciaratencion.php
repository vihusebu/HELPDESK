...
function get_format($df) {
 ...
 $sumatotal=0;
 $cantidadTotal=0;
  $consultatotal="SELECT tic.*, sol.fecha as fechasolucion, sol.horacreacion as horasolucion
                                            FROM ticket as tic
                                            INNER JOIN solucion sol on sol.idticket = tic.idticket
                                            WHERE tic.activo=1 and tic.estado IN(2,3) and tic.fecha BETWEEN '$fechaini' and '$fechafin'
                                            ORDER BY tic.fecha DESC";  

  foreach($ticket->sql($consultatotal) as $f3) 
  {
    //$date1 = new DateTime("2015-02-14 10:00:00");
    //$date2 = new DateTime("2015-02-15 10:00:00");
    $date1 = new DateTime($f3['fecha'].' '.$f3['horacreacion']);
    $date2 = new DateTime($f3['fechasolucion'].' '.$f3['horasolucion']);
...
<body>
    <div id="container" style="width: 100%;">
        ...
        <div style="text-align: center;"><p>TOTAL MINUTOS EN ESPERA DE ATENCION DE TICKET <?php echo $sumatotal.' '. $respue; ?></p></div>
    </div>
    <script>
        ...
        var barChartData = {
            labels: [
                      <?php 
                       $consultaNombres="SELECT tic.*, sol.fecha as fechasolucion, sol.horacreacion as horasolucion
                                            FROM ticket as tic
                                            INNER JOIN solucion sol on sol.idticket = tic.idticket
                                            WHERE tic.activo=1 and tic.estado IN(2,3) and tic.fecha BETWEEN '$fechaini' and '$fechafin'
                                            ORDER BY tic.fecha DESC";//estado=2 en proceso (iniciado el ticket)
                           foreach($ticket->sql($consultaNombres) as $f2)
                           {
                            $date1 = new DateTime($f2['fecha'].' '.$f2['horacreacion']);
                            $date2 = new DateTime($f2['fechasolucion'].' '.$f2['horasolucion']);
                            $diff = $date1->diff($date2);                                      
            ...
            datasets: [{
                ...
                data: [
                    <?php 
                        $consultameses="SELECT tic.*, sol.fecha as fechasolucion, sol.horacreacion as horasolucion
                                            FROM ticket as tic
                                            INNER JOIN solucion sol on sol.idticket = tic.idticket
                                            WHERE tic.activo=1 and tic.estado IN(2,3) and tic.fecha BETWEEN '$fechaini' and '$fechafin'
                                            ORDER BY tic.fecha DESC";
                           foreach($ticket->sql($consultameses) as $f)
                            {                                            
                                $date1 = new DateTime($f['fecha'].' '.$f['horacreacion']);
                                $date2 = new DateTime($f['fechasolucion'].' '.$f['horasolucion']);
                                $diff = $date1->diff($date2);
                                // 38 minutes to go [number is variable]
                                $minutototal=( ($diff->days * 24 ) * 60 ) + ( $diff->i );
         ...
         window.onload = function() {
            ...
            window.myBar = new Chart(ctx, {
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: 'TIEMPO DE ESPERA PARA INICIAR LA ATENCIÓN DE OPERADOR DE TICKET EN LA FECHAS <?php echo $fechaini ?> a <?php echo $fechafin ?>'
                    }
                }
            });
